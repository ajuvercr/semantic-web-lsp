...
export async function activate() {
    ...
    const virtualDocs: { [id: string]: string } = {};
    ...
    client.onRequest("workspace/applyEdit", (e: ApplyWorkspaceEditParams) => {
        const changes = e.edit.changes ? Object.entries(e.edit.changes) : [];
        let applied = false
        for (const edits of changes) {
            const key = edits[0].toString();

            for (const edit of edits[1]) {
                virtualDocs[key.toString()] = edit.newText
            }
            const uri = vscode.Uri.parse(edits[0])
            emitter.fire(uri);
            applied = true
        }
        return { applied };
